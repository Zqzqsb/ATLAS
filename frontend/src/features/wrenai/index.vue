...
import WrenOverview from './components/overview/WrenOverview.vue'
import WrenModuleDetail from './components/module/WrenModuleDetail.vue'
import { getWrenFlow, type ArchNode } from './model/wren'
...
const activeFlow = computed(() => getWrenFlow(activeFlowId.value))
...
<template>
  <div class="wren-page bg-gradient-to-br from-slate-100 via-emerald-50/40 to-teal-50/30">
    ...
    <div class="bg-white/80 backdrop-blur border-b border-gray-200/80 px-8 py-3 flex-shrink-0">
      <div class="max-w-5xl mx-auto flex items-center gap-2 text-sm">
        <button
          ...
          @click="back"
        >
          <div class="i-lucide-bird text-base" />
          WrenAI 全景架构
        </button>
    ...
    <div ref="stageRef" class="relative flex-1 overflow-hidden">
      ...
      >
        <WrenModuleDetail v-if="activeFlow" :flow="activeFlow" @back="back" />
      </div>
    </div>
  </div>
</template>
