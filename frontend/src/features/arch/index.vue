...
import ArchOverview from './components/overview/ArchOverview.vue'
import ModuleDetail from './components/module/ModuleDetail.vue'
import { getFlow } from './model/flows'
...
const activeFlow = computed(() => getFlow(activeFlowId.value))

function drillInto(node: ArchNode, ev: MouseEvent) {
  if (!node.flow) return
  // Compute the zoom origin from the clicked node's position within the stage,
  // so the detail view appears to grow out of that exact node.
...
<template>
  <div class="arch-page bg-gradient-to-br from-slate-100 via-violet-50/40 to-indigo-50/30">
    ...
    <div class="bg-white/80 backdrop-blur border-b border-gray-200/80 px-8 py-3 flex-shrink-0">
      <div class="max-w-5xl mx-auto flex items-center gap-2 text-sm">
        <button
          ...
          @click="back"
        >
          <div class="i-lucide-boxes text-base" />
          全景架构
        </button>
    ...
    <!-- Stage: overview + module layered, cross-faded with a zoom focus -->
    <div ref="stageRef" class="relative flex-1 overflow-hidden">
      ...
      >
        <ModuleDetail v-if="activeFlow" :flow="activeFlow" @back="back" />
      </div>
    </div>
  </div>
</template>
