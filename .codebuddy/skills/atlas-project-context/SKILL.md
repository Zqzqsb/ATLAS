...
description: >
  ATLAS 项目全景知识 — VLDB Demo Track 自适应 Text-to-SQL 系统。
  This skill should be used when working on any part of the ATLAS codebase,
...
```
backend/
  cmd/atlas-server/main.go  — 入口，初始化所有 service
  internal/
...
| atlas_evolution | 2(初始) | 自维持演示(5阶段DDL演进) | ✅ 就绪 |
| TPC-H | 30+ | 两阶段 Linking 效率演示 | ❌ 尚未创建 |

## 演示策略 — 一库一创新点

- Spider 库 → 推理管线 + Rich Context 对 SQL 生成的指导价值
- atlas_evolution → Agent 自维持（5 阶段 DDL 演进闭环）
