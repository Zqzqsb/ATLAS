...
import SnowOverview from './components/overview/SnowOverview.vue'
import SnowModuleDetail from './components/module/SnowModuleDetail.vue'
import { getSnowFlow } from './model/flows'
...
const activeFlow = computed(() => getSnowFlow(activeFlowId.value))
...
<template>
  <div class="snow-page bg-gradient-to-br from-slate-100 via-sky-50/50 to-blue-50/30">
    <div class="bg-white/80 backdrop-blur border-b border-gray-200/80 px-8 py-3 flex-shrink-0">
      <div class="max-w-5xl mx-auto flex items-center gap-2 text-sm">
        <button
          ...
          @click="back"
        >
          <div class="i-lucide-snowflake text-base" />
          Snowflake 全景架构
        </button>
    ...
    <div ref="stageRef" class="relative flex-1 overflow-hidden">
      ...
      >
        <SnowModuleDetail v-if="activeFlow" :flow="activeFlow" @back="back" />
      </div>
    </div>
  </div>
</template>
