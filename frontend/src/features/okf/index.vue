...
import OkfOverview from './components/overview/OkfOverview.vue'
import OkfModuleDetail from './components/module/OkfModuleDetail.vue'
import { OKF_LAYERS, getOkfFlow, type ArchNode } from './model/okf'
...
const activeFlow = computed(() => getOkfFlow(activeFlowId.value))
...
<template>
  <div class="okf-page bg-gradient-to-br from-slate-100 via-violet-50/40 to-emerald-50/30">
    <div class="bg-white/80 backdrop-blur border-b border-gray-200/80 px-8 py-3 flex-shrink-0">
      <div class="max-w-5xl mx-auto flex items-center gap-2 text-sm">
        <button
          ...
          @click="back"
        >
          <div class="i-lucide-file-stack text-base" />
    ...
    <div ref="stageRef" class="relative flex-1 overflow-hidden">
      ...
      >
        <OkfModuleDetail v-if="activeFlow" :flow="activeFlow" @back="back" />
      </div>
    </div>
  </div>
</template>
